...
V 1.5
*/
define('API_KEY','412808569:AAELbXXKZqPEp1MB3m3Anloo6DhDtonK1AQ');//addtoken
//-----------------------------------------------------------------------------------------
//فانکشن MrPHPBot :
...
$token = '412808569:AAELbXXKZqPEp1MB3m3Anloo6DhDtonK1AQ';//add token
...
if($textmassage=="/start" && $tc == "private"){
  sendAction($chat_id, 'typing');
	...
	'chat_id'=>$chat_id,
	'text'=>"⚜سلام⚜
♨️به روبات ضد لینک خوش امدید.جهت استفاده از عملکرد روبات به گروه خود دعوت کنید.
و سپس دستور /add را وارد کنید.
...
['text'=>"برگشت",'callback_data'=>'back']
  ...
  elseif($data=="back" && $fm == $owners2){
   ...
   [
['text'=>"کانال ما",'url'=>"https://telegram.me/antispamrobot_ch"]
  ],
    ]
    ])
    ]);
    ...
    elseif($data=="help" && $fm == $owners2){
     ...
     'message_id'=>$messageid,
             ...
             *[/]automatic [manage]* ➖ (مدیریت گروه به صورت خودکار توسط ربات)
			 [سازنده ](https://telegram.me/AntiSpam_rooobot)",
            'parse_mode'=>'MarkDown',
               'reply_markup'=>json_encode([
                   'inline_keyboard'=>[
                     [
...
elseif( $textmassage == "/leave" && $from_id == $Dev){
	sendmessage($chat_id,"به درخواست مدیر رفتم بای:|");
	MrPHPBot('leaveChat',[
	'chat_id'=>$chat_id,
	]);
...
if ($tc == 'group' | $tc == 'supergroup'){
 ...
 'text'=>"مقام:
  ...
  'parse_mode'=>'MarkDown',
     ...
     sendmessage($chat_id,"این عضو مقامی ندارد");
 }}
}
...
elseif(preg_match("'^(کسکش)(.*)'",$textmassage)){
preg_match("'^(کسکش)(.*)'",$textmassage,$match);
if ($tc == 'group' | $tc == 'supergroup'){
if ($lockfosh == "✔️"){
if ($status != "creator" && $status != "administrator"){
  save("data/$from_id/file.txt","none");
...
elseif(preg_match("'^(ننه صلواتی)(.*)'",$textmassage)){
preg_match("'^(ننه صلواتی)(.*)'",$textmassage,$match);
if ($tc == 'group' | $tc == 'supergroup'){
if ($lockfosh == "✔️"){
if ($status != "creator" && $status != "administrator"){
  save("data/$from_id/file.txt","none");
...
elseif(preg_match("'^(ننه)(.*)'",$textmassage)){
preg_match("'^(ننه)(.*)'",$textmassage,$match);
if ($tc == 'group' | $tc == 'supergroup'){
if ($lockfosh == "✔️"){
if ($status != "creator" && $status != "administrator"){
  save("data/$from_id/file.txt","none");
...
elseif(preg_match("'^(خواهرت)(.*)'",$textmassage)){
preg_match("'^(خواهرت)(.*)'",$textmassage,$match);
if ($tc == 'group' | $tc == 'supergroup'){
if ($lockfosh == "✔️"){
if ($status != "creator" && $status != "administrator"){
  save("data/$from_id/file.txt","none");
...
elseif(preg_match("'^(کص ننت)(.*)'",$textmassage)){
preg_match("'^(کص ننت)(.*)'",$textmassage,$match);
if ($tc == 'group' | $tc == 'supergroup'){
if ($lockfosh == "✔️"){
if ($status != "creator" && $status != "administrator"){
  save("data/$from_id/file.txt","none");
...
elseif(preg_match("'^(مادر جنده)(.*)'",$textmassage)){
preg_match("'^(مادر جنده)(.*)'",$textmassage,$match);
if ($tc == 'group' | $tc == 'supergroup'){
if ($lockfosh == "✔️"){
if ($status != "creator" && $status != "administrator"){
  save("data/$from_id/file.txt","none");
...
elseif(preg_match("'^(بصیک)(.*)'",$textmassage)){
preg_match("'^(بصیک)(.*)'",$textmassage,$match);
if ($tc == 'group' | $tc == 'supergroup'){
if ($lockfosh == "✔️"){
if ($status != "creator" && $status != "administrator"){
  save("data/$from_id/file.txt","none");
...
elseif(preg_match("'^(کونی)(.*)'",$textmassage)){
preg_match("'^(کونی)(.*)'",$textmassage,$match);
if ($tc == 'group' | $tc == 'supergroup'){
if ($lockfosh == "✔️"){
if ($status != "creator" && $status != "administrator"){
  save("data/$from_id/file.txt","none");
...
}
{
   MrPHPBot('answerInlineQuery', [
        ...
        'results' => json_encode([[
            'type' => 'article',
             'thumb_url'=>"http://uupload.ir/files/9fdv_431836349_78430.jpg",
...
و...
➖➖➖
[🌟@antispamrobot_ch🌟](https://telegram.me/antispamrobot_ch)"],
            'reply_markup' => [
                'inline_keyboard' => [
                    [
                        ['text' => "عضویت در ربات", 'url' => 'https://telegram.me/AntiSpam_rooobot']
                    ],
                    [
